...
function App() {
  ...
  const updateFilm = useCallback(
    (newFilm) => {
      if (
        newFilm &&
        isNaN(Number(newFilm)) &&
        newFilm.indexOf("kinopoisk.ru") > -1 &&
        newFilm.match(/kinopoisk.ru\/(?:film|series)\/([0-9]+)\//)
      ) {
        const kp = newFilm.match(
          /kinopoisk.ru\/(?:film|series)\/([0-9]+)\//,
        )[1];
        window.history.pushState(
          {},
          "",
          window.location.origin + "?f=" + encodeURI(kp) + (searchParams.has('test') ? '&test' : ''),
        );
        setFilm(kp);
        ...
        window.history.pushState(
          {},
          "",
          window.location.origin + "?f=" + encodeURI(newFilm) + (searchParams.has('test') ? '&test' : ''),
        );
        setFilm(newFilm);
        setFilmIsString(false);
      } else {
